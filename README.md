...
**1. Record what production looks like**, right after your backup runs
(read-only, safe for production and read replicas):

```sh
pgdrill baseline --db "$PRODUCTION_URL" -o baseline.json
```
...
(`pg15`, `pg16`, `pg17`). Exit code `0` = pass, `1` = the backup failed a check,
`2` = pgdrill itself couldn't run.
...
```sh
gem install pgdrill          # Ruby >= 3.0, no other gem dependencies
...
`--target postgresql://user@host/postgres` (needs `CREATE DATABASE`; the
restored database is dropped afterwards unless you pass `--keep`).
...
- **Credentials** are passed to `psql` through environment variables, never on
  the command line.
- **Nothing leaves your machine**: no telemetry, no uploads. The throwaway
  server and its data are deleted after each run.
- The throwaway server runs with `fsync=off` for speed; it is never used for anything else.
