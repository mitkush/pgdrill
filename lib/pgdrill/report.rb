...
module Pgdrill
  ...
  class Report
    def initialize(backup:, baseline_source:, baseline:, restore:, restored:, findings:, options:, now: Time.now.utc)
      @backup, @baseline_source, @baseline = backup, baseline_source, baseline
    ...
    def to_h
      ...
      base_newest = @baseline && Baseline.newest(@baseline)
      {
        ...
        "pgdrill" => VERSION,
        "ran_at" => @now.iso8601,
        "backup" => { "path" => @backup.path, "format" => @backup.format.to_s, "bytes" => @backup.size,
                      "sha256" => @options[:sha256] ? @backup.sha256 : nil,
    ...
    def to_text
      ...
      out = []
      out << "pgdrill #{VERSION}"
      out << "backup    #{b['path']} (#{b['format']}, #{human_bytes(b['bytes'])}, from Postgres #{b['dumped_from'] || '?'})"
      out << "baseline  #{@baseline ? "#{@baseline_source} (captured #{@baseline['captured_at']})" : 'none (self-consistency checks only; pass --baseline for full checks)'}"
      out << ""
    ...
    def line(label, text) = "#{label.ljust(10)}#{text}"
